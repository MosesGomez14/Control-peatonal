...
def people_counter():
	...
	if not args.get("input", False):
		logger.info("Comenzando app de monitoreo con video en vivo..")
		vs = VideoStream(config["url"]).start()
		time.sleep(2.0)

	# otherwise, grab a reference to the video file
	else:
		logger.info("Comenzando app de monitoreo con archivo de video..")
	...
	while True:
		...
		cv2.putText(frame, "-Linea de ingreso - Entrada-", (10, H - ((i * 20) + 200)),
				...
				if not to.counted:
					...
					# center line, count the object
						...
						if sum(total) >= config["Threshold"]:
							cv2.putText(frame, "-ALERTA: EXCESO DE PERSONAS EN ALMACEN-", (10, frame.shape[0] - 80),
		...
		info_total = [
		("Personas dentro: ", ', '.join(map(str, total))),
		]
		...
		cv2.imshow("Monitoreo/Ingreso de personas", frame)
